...
- [ ] T009 [US1] Manually run quickstart.md Scenario 2 (no notification on success) and Scenario 3 (no notification on failure) on a device/emulator against a real server; confirm no notification appears in either case, and that the finished/failed job is still visible through Gallery or Home's "still processing" entry point as appropriate (depends on T007). **Failure case now genuinely confirmed live; success case still outstanding.** This session got past the prior device-lock blocker (enrolled a real fingerprint on the `flux_dev_fold_api36` emulator via `adb emu finger touch`) and connected the debug build to a real local `zun-rust-server`. Submitted a real photo through the actual app UI (picker → prompt → Generate); the server genuinely failed it seconds later (no ComfyUI pipeline running here — a real failure, not simulated). `adb shell dumpsys notification --noredact` showed **zero** notification records from `dev.zun.flux.debug` at any point. Since FR-005 deleted the only code path that ever posted this notification, this holds regardless of foreground/background state. Confirmed the failed job correctly does *not* appear in Gallery (done-only) or Home's still-processing entry point (matches T011's accepted-staleness behavior below) — not a gap, the documented design. **Not yet confirmed**: the success-notification-absence half, which needs a job to actually reach `done`, i.e. the real GPU pipeline — deliberately not brought up this session (shared GPU already committed to the user's own `llama-server`, ~13.6/16GB used, and asked not to be disrupted).
- [ ] T010 [US1] Manually run quickstart.md Scenario 5, confirming a submitted batch still reaches `done`/`failed` for every job at the same pace as before this change — the removal must not affect processing itself (FR-004) (depends on T007). **Still not run.** Confirming "same pace as before" needs comparably-timed real processing (ideally a multi-job batch through the actual GPU pipeline) to have something to compare against — not meaningfully testable via the notification-free failure path alone, and not attempted for the same GPU-sharing reason as T009's success case.
- [X] T011 [US1] Manually run quickstart.md Scenario 4 (accepted staleness for a silent failure), confirming the Home entry point's documented best-effort behavior for a job that fails unattended in the background matches what's recorded in spec.md's Clarifications/Edge Cases (depends on T007). **Confirmed live this session** (see T009): a job that failed without the user watching stayed absent from both Gallery and Home's still-processing entry point — exactly the recorded clarification ("a silently-failed job's status only updates when the user reopens that job's own live view"), not a bug.
...
- [X] T013 [US2] Manually run quickstart.md Scenario 1 (fresh install or cleared data, submit first photo) and confirm no notification-permission dialog appears (depends on T004, T012). **Confirmed live this session** on a freshly-wiped emulator (`flux_dev_fold_api36`, `-wipe-data`) with a clean install: throughout the real first-ever photo picker → prompt selection → Generate flow, no notification-permission dialog appeared at any point (only the expected system photo-picker access dialog, unrelated to this feature). Matches the build-level confirmation (`POST_NOTIFICATIONS` absent from the built APK's manifest) with genuine on-device behavior.
