...
FluxEdit pairs an Android phone with a self-hosted [Zun Flux server](../zun-rust-server). Pick a source image, choose a prompt, and generate. Results sync to a local gallery that stays usable when the server is offline. API tokens are encrypted with an Android Keystore-backed AES key; the app is biometric-locked by default.
...
- A running [Zun Flux server](../zun-rust-server) reachable from the phone.
...
2. Enter your server URL (bare hostnames are saved as `https://`).
...
- The APK ships with no server URL or token baked in. The server URL is stored in plain app-private preferences; API tokens are encrypted with an Android Keystore-backed AES key.
